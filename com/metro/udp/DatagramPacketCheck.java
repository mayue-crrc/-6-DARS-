...
// @author: Deng Ran
// @description: used to check the validation of the datagram
public class DatagramPacketCheck {	
	public final static int PROCESS_FRAME = 0x0A01;
	public final static int FAULT_FRAME = 0x0A02;
	
	// @author: Deng Ran
	// @description: these data are used to calculate the crc code
	...
	// @author: Deng Ran
	// @description: the size of the datagram should be larger than the threshold value
	public static boolean checkSize(DatagramPacket packet) {
		if (packet.getLength() < 400) {
			return false;
		} else {
			return true;
		}
	...
	// @author: Deng Ran
	// @description: check the project number in the message header
	public static boolean checkProjectNumber(DatagramPacket packet) {
		if (DataAccess.getU8(packet.getData(), 0) == 1) {
			return true;
		} else {
			return false;
		}
	...
	// @author: Deng Ran
	// @description: check the message type
	// 		0x0A01: real time message; 0x0A02: fault message; 0x0201: feedback message
	public static boolean checkMessageType(DatagramPacket packet) {
		if (DataAccess.getU16(packet.getData(), 14) == 0x0A01) {
			return true;
		} else if (DataAccess.getU16(packet.getData(), 14) == PROCESS_FRAME) {
			return true;
		} else if (DataAccess.getU16(packet.getData(), 14) == FAULT_FRAME) {
			return true;
		} else {
			return false;
		}
	...
	// check if it is a process frame
	public static boolean isProcessFrame(byte[] data) {
		if (UniversalService.getU16(data, 18) == PROCESS_FRAME) {
			return true;
		} else {
			return false;
		}
	...
	// check if it is a fault frame
	public static boolean isFaultFrame(byte[] data) {
		if (UniversalService.getU16(data, 18) == FAULT_FRAME) {
			return true;
		} else {
			return false;
		}
	...
	// @author: Deng Ran
	// @description: check the crc code
	public static boolean checkCrcCode(byte[] data) {
		if (DataAccess.getU16(data, data.length - 2) == getCrcCode(data, 0, data.length - 2, 0xFFFF)) {
			return true;
		} else {
			return false;
		}
	...
	// @author: Deng Ran
	// @description: calculate the crc code used the array at the begining of the class
	public static int getCrcCode(byte[] data, int offset, int len, int preval) {
